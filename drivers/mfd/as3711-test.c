/*
 * as3711-test.c - testing the AS3711 driver
 *
				   ...
				   size_t count, loff_t * ppos)
{
	...
	if (strcmp(buf, "enable") == 0) {
	...
	} else if ((pos = strstr(buf, "set_voltage")) != NULL) {
	...
	} else if ((pos = strstr(buf, "set_current")) != NULL) {
		...
		return buf_size;
	} else if ((pos = strstr(buf, "set_mode")) != NULL) {
				      ...
				      size_t count, loff_t * ppos)
{
	...
	/* modified by byron */
	reg = regulator_get_exclusive(NULL, buf);
	if (IS_ERR(reg)) {
	...
	fdata =
	    (struct dyn_file_data *)kmalloc(sizeof(struct dyn_file_data),
					    GFP_KERNEL);
	if (!fdata) {
	...
	reg_list_entry =
	    (struct regulator_list *)kmalloc(sizeof(struct regulator_list),
					     GFP_KERNEL);
	if (!reg_list_entry) {
...
static int as3711_test_probe(void)
{
	INIT_LIST_HEAD(&regulators.list);

	debug_dir = debugfs_create_dir("as3711", NULL);
	if (!debug_dir) {
...
static int as3711_test_remove(void)
{
...
static int __init as3711_test_init(void)
{
	as3711_test_probe();
	return 0;
}

static void __exit as3711_test_exit(void)
{
	as3711_test_remove();
}

module_init(as3711_test_init);
module_exit(as3711_test_exit);

MODULE_DESCRIPTION("AS3711 PMIC testing");
MODULE_LICENSE("GPL");
